    ...
    if (this.tablaCamion.length === 0 && this.tablaVagon.length === 0) {
      ...
      this.tablaCamion.forEach((fila) => {
        worksheet.addRow([
          ...
          fila.camion,
          fila.batea,
          Number(fila.brutoOrigen),
          Number(fila.taraOrigen),
          Number(fila.netoHumedad),
          fila.fDestino,
          fila.hDestino,
          Number(fila.brutoDestino),
          Number(fila.taraDestino),
          Number(fila.netoHumedoDestino),
          Number(fila.diferenciaHumeda),
          Number(fila.diferenciaSeca),
          Number(fila.CuFino),
          fila.estado,
          fila.bodega,
        ]);
